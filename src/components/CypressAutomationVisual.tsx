...
import { SiCypress, SiPostman, SiGooglechrome, SiFirefox, SiGithub } from 'react-icons/si';
...
const CARDS: TechCardSpec[] = [
  // ── API Testing — top-centre (distance 240px) ──
  {
    id: 'postman',
    title: 'API Testing',
    style: { top: 0, left: CX - CW },      // cy = 60
    cx: CX,
    cy: CH,                                 // 60
    delay: 0.4,
    icon: <SiPostman size={44} color="#FF6C37" />,
  },
  // ── Chrome — upper-left ──
  {
    id: 'chrome',
    ...
    cy: 70 + CH,                            // 130
    delay: 0,
    icon: <SiGooglechrome size={44} color="#4285F4" />,
  },
  // ── Firefox — upper-right ──
  {
    id: 'firefox',
    ...
    cy: 70 + CH,                            // 130
    delay: 0.8,
    icon: <SiFirefox size={44} color="#FF7139" />,
  },
  // ── GitHub — lower-left ──
  {
    id: 'github',
  ...
  // ── CI/CD — bottom-centre (distance 240px) ──
  {
    id: 'cicd',
    title: 'CI/CD',
    style: { bottom: 0, left: CX - CW },   // cy = 540
    cx: CX,
    cy: H - CH,                             // 540
    delay: 1.6,
    icon: <GitBranch size={44} className="text-[#2563EB]" strokeWidth={1.5} />,
  },
  // ── Dashboard — lower-right ──
  {
    ...
    cy: H - 70 - CH,                        // 470
    delay: 2.0,
    icon: <BarChart3 size={44} className="text-[#2563EB]" strokeWidth={1.5} />,
  },
];

// ─── Cypress Center Logo ───────────────────────────────────────────────────────
const CypressCenter: React.FC = () => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
    <SiCypress size={80} color="#00BF88" />
...
export const CypressAutomationVisual: React.FC = () => {
  return (
    <div className="w-full max-w-[700px] aspect-[7/6] mx-auto relative flex items-center justify-center">
      ...
      >
        <defs>
          <radialGradient id="cy-node-glow">
        ...
        {CARDS.map((card) => {
          ...
          return (
            <g key={card.id}>
              ...
              />
              <circle cx={dotX} cy={dotY} r="9" fill="url(#cy-node-glow)" />
              <motion.circle
      ...
      {CARDS.map((card) => (
        ...
        >
          ...
          >
            {card.icon}
            <span style={{ fontSize: 13, fontWeight: 600, color: '#0F172A', fontFamily: 'Inter, sans-serif', letterSpacing: '0.01em' }}>
              {card.title}
            </span>
          </motion.div>
        </motion.div>
...
export default CypressAutomationVisual;
